...
local function parse(arg)
    ...
    cmd:option('-imageDir', '/home/data/datasets/mpii/images', 'Image directory')
    ...
    cmd:text()
    cmd:option('-nEpochs',          5000, 'Total number of epochs to run')
    cmd:option('-trainIters',        120, 'Number of train iterations per epoch')
    cmd:option('-trainBatch',         16, 'Mini-batch size')
    cmd:option('-validIters',         10, 'Number of validation iterations per epoch')
    cmd:option('-validBatch',         10, 'Mini-batch size for validation')
    cmd:option('-nValidImgs',        100, 'Number of images to use for validation')
    cmd:text()
